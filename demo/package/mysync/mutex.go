...
// handleConfig 代表处理流程配置的类型
type handleConfig struct {
	handler   singleHandler // 单次处理函数
	goNum     int           // 需要启用的goroutine数
	number    int           // 单个goroutine中的处理次数
	interval  time.Duration // 单个goroutine中的处理间隔时间
	counter   int           // 数据量计数器 以字节为单位
	counterMu sync.RWMutex  // 数据量计数器专用的互斥锁
}

// count 会增加计数器的值， 并返回增加后的数值
func (hc *handleConfig) count(increment int) int {
	hc.counterMu.Lock()
	...
	for i := 1; i <= writingConfig.goNum; i++ {
		go func(i int) {
			...
			for j := 1; j < writingConfig.number; j++ {
				time.Sleep(time.Duration(writingConfig.interval))
	...
	for i := 1; i <= writingConfig.goNum; i++ {
		go func(i int) {
			...
			for j := 1; j <= readingConfig.number; j++ {
				time.Sleep(time.Duration(readingConfig.interval))
				var data string
				var n int
				var err error
				for {
